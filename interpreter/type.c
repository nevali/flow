...
struct ValueType *
TypeAdd(Picoc * pc, struct ParseState *Parser, struct ValueType *ParentType, enum BaseType Base, int ArraySize, const char *Identifier, int Sizeof, int AlignBytes)
{
...
struct ValueType *
TypeGetMatching(Picoc * pc, struct ParseState *Parser, struct ValueType *ParentType, enum BaseType Base, int ArraySize, const char *Identifier, int AllowDuplicates)
{
	int	Sizeof;
	int	AlignBytes;
...
int
TypeStackSizeValue(struct Value *Val)
{
...
int
TypeSizeValue(struct Value *Val, int Compact)
{
	if (IS_INTEGER_NUMERIC(Val) && !Compact)
		return sizeof(ALIGN_TYPE);	/* allow some extra room for
						 * type extension */
...
int
TypeSize(struct ValueType *Typ, int ArraySize, int Compact)
{
...
/* add a base type */
void
TypeAddBaseType(Picoc * pc, struct ValueType *TypeNode, enum BaseType Base, int Sizeof, int AlignBytes)
{
	TypeNode->Base = Base;
...
TypeInit(Picoc * pc)
{
	...
	TypeAddBaseType(pc, &pc->TypeType, Type_Type, sizeof(double), (char *)&da.y - &da.x);	/* must be large enough
												 * to cast to a double */
#else
...
void
TypeCleanupNode(Picoc * pc, struct ValueType *Typ)
{
	...
	for (SubType = Typ->DerivedTypeList; SubType != NULL; SubType = NextSubType)
	{
		...
		if (SubType->OnHeap)
		{
			/* if it's a struct or union deallocate all the member
			 * values */
...
void
TypeParseStruct(struct ParseState *Parser, struct ValueType **Typ, int IsStruct)
{
	...
	*Typ = TypeGetMatching(pc, Parser, &Parser->pc->UberType, IsStruct ? TypeStruct : TypeUnion, 0, StructIdentifier, TRUE);
	...
	do
	{
		...
		{
			/* union members always start at 0, make sure it's big
...
struct ValueType *
TypeCreateOpaqueStruct(Picoc * pc, struct ParseState *Parser, const char *StructName, int Size)
{
...
void
TypeParseEnum(struct ParseState *Parser, struct ValueType **Typ)
{
	struct Value *LexValue;
...
int
TypeParseFront(struct ParseState *Parser, struct ValueType **Typ, int *IsStatic, enum LexToken *InitialToken)
{
	...
	*Typ = NULL;
	...
	Token = (InitialToken ? (*InitialToken) : LexGetToken(Parser, NULL, FALSE));
	if (Token == TokenLeftSquareBracket)
	{
		/* XXX save attributes */
		TypeParseAttributes(Parser);
		if (InitialToken)
		{
			*InitialToken = LexGetToken(Parser, NULL, FALSE);
		}
	}
	...
	switch (Token)
	{
		...
		case TokenInterface:
			if (*Typ != NULL)
				ProgramFail(Parser, "bad interface declaration");
			TypeParseInterface(Parser, Typ);
			break;
		case TokenModule:
			if (*Typ != NULL)
				ProgramFail(Parser, "bad module declaration");
			TypeParseModule(Parser, Typ);
			break;
		...
		case TokenIdentifier:
			/* we already know it's a typedef-defined type because
			 * we got here */
...
/* parse a type - the part at the end after the identifier. eg. array
 * specifications etc. */
struct ValueType *
TypeParseBack(struct ParseState *Parser, struct ValueType *FromType)
{
	enum LexToken Token;
	struct ParseState Before;
...
/* parse a type - the part which is repeated with each identifier in a
 * declaration list */
void
...
void
TypeParse(struct ParseState *Parser, struct ValueType **Typ, char **Identifier, int *IsStatic)
{
	struct ValueType *BasicType;

	TypeParseFront(Parser, &BasicType, IsStatic, NULL);
	TypeParseIdentPart(Parser, BasicType, Typ, Identifier);
}

/* check if a type has been fully defined - otherwise it's just a forward
 * declaration */
int
TypeIsForwardDeclared(struct ParseState *Parser, struct ValueType *Typ)
{
	if (Typ->Base == TypeArray)
		return TypeIsForwardDeclared(Parser, Typ->FromType);

	if ((Typ->Base == TypeStruct || Typ->Base == TypeUnion ||
			Typ->Base == TypeInterface || Typ->Base == TypeClass ||
			Typ->Base == TypeModule) && Typ->Members == NULL)
		return TRUE;

...
#define MAX_CHAR_VALUE 255	       /* maximum value which can be
				        * represented by a "char" data type */
...
LexInit(Picoc * pc)
{
	...
	for (Count = 0; (unsigned long) Count < sizeof(ReservedWords) / sizeof(struct ReservedWord); Count++)
	{
...
enum LexToken
LexCheckReservedWord(Picoc * pc, const char *Word)
{
	struct Value *val;
...
enum LexToken
LexGetNumber(Picoc * pc, struct LexState *Lexer, struct Value *Value)
{
	long	Result = 0;
	long	Base = 10;
...
enum LexToken
LexGetWord(Picoc * pc, struct LexState *Lexer, struct Value *Value)
{
...
unsigned char
LexUnEscapeCharacterConstant(const char **From, const char *End, unsigned char FirstChar, int Base)
{
...
unsigned char
LexUnEscapeCharacter(const char **From, const char *End)
{
	unsigned char ThisChar;

	while (*From != End && **From == '\\' &&
		&(*From)[1] != End && (*From)[1] == '\n')
		(*From) += 2;	       /* skip escaped end of lines with LF
				        * line termination */

	while (*From != End && **From == '\\' &&
		&(*From)[1] != End && &(*From)[2] != End && (*From)[1] == '\r' && (*From)[2] == '\n')
		(*From) += 3;	       /* skip escaped end of lines with CR/LF
				        * line termination */
...
enum LexToken
LexGetStringConstant(Picoc * pc, struct LexState *Lexer, struct Value *Value, char EndChar)
{
	int	Escape = FALSE;
	const char *StartPos = Lexer->Pos;
	const char *EndPos;
...
enum LexToken
LexGetCharacterConstant(Picoc * pc, struct LexState *Lexer, struct Value *Value)
{
...
void
LexSkipComment(struct LexState *Lexer, char NextChar, enum LexToken *ReturnToken)
{
	if (NextChar == '*')
	{
...
enum LexToken
LexScanGetToken(Picoc * pc, struct LexState *Lexer, struct Value **Value)
{
	char	ThisChar;
	char	NextChar;
	enum LexToken GotToken = TokenNone;

	/* handle cases line multi-line comments or string constants which mess
	 * up the line count */
...
int
LexTokenSize(enum LexToken Token)
{
	switch (Token)
	{
...
/* produce tokens from the lexer and return a heap buffer with the result -
 * used for scanning */
void   *
LexTokenise(Picoc * pc, struct LexState *Lexer, int *TokenLen)
{
	enum LexToken Token;
	void   *HeapMem;
...
void   *
LexAnalyse(Picoc * pc, const char *FileName, const char *Source, int SourceLen, int *TokenLen)
{
...
void
LexInitParser(struct ParseState *Parser, Picoc * pc, const char *SourceText, void *TokenSource, char *FileName, int RunIt, int EnableDebugger)
{
	Parser->pc = pc;
...
enum LexToken
LexGetRawToken(struct ParseState *Parser, struct Value **Value, int IncPos)
{
		...
		if (Parser->FileName == pc->StrEmpty && (pc->InteractiveHead == NULL || Token == TokenEOF))
		{
			...
			if (pc->InteractiveHead == NULL || (unsigned char *)Parser->Pos == &pc->InteractiveTail->Tokens[pc->InteractiveTail->NumBytes - TOKEN_DATA_OFFSET])
			{
				...
				if (PlatformGetLine(&LineBuffer[0], LINEBUFFER_MAX, Prompt) == NULL)
					return TokenEOF;
				/* put the new line at the end of the linked
...
void
LexHashIncPos(struct ParseState *Parser, int IncPos)
{
	if (!IncPos)
...
void
LexHashIfdef(struct ParseState *Parser, int IfNot)
{
	/* get symbol to check */
...
void
LexHashIf(struct ParseState *Parser)
{
	/* get symbol to check */
	struct Value *IdentValue;
...
void
LexHashElse(struct ParseState *Parser)
{
	if (Parser->HashIfEvaluateToLevel == Parser->HashIfLevel - 1)
		Parser->HashIfEvaluateToLevel++;	/* #if was not active,
							 * make this next
							 * section active */
...
void
LexHashEndif(struct ParseState *Parser)
{
	if (Parser->HashIfLevel == 0)
...
void
LexPrintToken(enum LexToken Token)
{
	char   *TokenNames[] = {
		 /* 0x00 */ "None",
		 /* 0x01 */ "Comma",
...
enum LexToken
LexGetToken(struct ParseState *Parser, struct Value **Value, int IncPos)
{
	enum LexToken Token;
	int	TryNextToken;
...
enum LexToken
LexRawPeekToken(struct ParseState *Parser)
{
...
void
LexToEndOfLine(struct ParseState *Parser)
{
	while (TRUE)
	{
...
void   *
LexCopyTokens(struct ParseState *StartParser, struct ParseState *EndParser)
{
	int	MemSize = 0;
	int	CopySize;
...
/* indicate that we've completed up to this point in the interactive input and
 * free expired tokens */
void
LexInteractiveClear(Picoc * pc, struct ParseState *Parser)
{
 ...
 * free expired tokens */
void
LexInteractiveCompleted(Picoc * pc, struct ParseState *Parser)
{
...
void
LexInteractiveStatementPrompt(Picoc * pc)
{

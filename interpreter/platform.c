/* picoc's interface to the underlying platform. most platform-specific code is
 * in platform/platform_XX.c and platform/library_XX.c */
...
void
PicocInitialise(Picoc * pc, int StackSize)
{
...
void
PicocCallMain(Picoc * pc, int argc, char **argv)
{
...
void
PrintSourceTextErrorLine(IOFILE * Stream, const char *FileName, const char *SourceText, int Line, int CharacterPos)
{
	int	LineCount;
	const char *LinePos;
	const char *CPos;
	...
	{
		/* assume we're in interactive mode - try to make the arrow
		 * match up with the input text */
...
/* exit with a message */
void
ProgramFail(struct ParseState *Parser, const char *Message,...)
{
	va_list	Args;
...
void
ProgramFailNoParser(Picoc * pc, const char *Message,...)
{
	va_list	Args;
...
void
AssignFail(struct ParseState *Parser, const char *Format, struct ValueType *Type1, struct ValueType *Type2, int Num1, int Num2, const char *FuncName, int ParamNo)
{
...
void
LexFail(Picoc * pc, struct LexState *Lexer, const char *Message,...)
{
	va_list	Args;
...
void
PlatformPrintf(IOFILE * Stream, const char *Format,...)
{
	va_list	Args;
...
void
PlatformVPrintf(IOFILE * Stream, const char *Format, va_list Args)
{
	const char *FPos;
 ...
 * should be initialised to "XX0000" where XX can be any characters */
char   *
PlatformMakeTempName(Picoc * pc, char *TempNameBuffer)
{
	int	CPos = 5;

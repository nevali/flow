...
/* If the destination is not float, we can't assign a floating value to it, we
 * need to convert it to integer instead */
...
/* NOTE: the order of this array must correspond exactly to the order of these
 * tokens in enum LexToken */
static struct OpPrecedence OperatorPrecedence[] =
{
	 ...
	 /* TokenShiftLeftAssign, */ {0, 0, 2, "<<="},	/* TokenShiftRightAssign,
	    */ {0, 0, 2, ">>="}, /* TokenArithmeticAndAssign, */ {0, 0, 2, "&="},
	 /* TokenArithmeticOrAssign, */ {0, 0, 2, "|="},	/* TokenArithmeticExorAss
	    ign, */ {0, 0, 2, "^="},
	 ...
	 /* TokenLeftSquareBracket, */ {0, 0, 15, "["},	/* TokenRightSquareBracke
	    t, */ {0, 15, 0, "]"}, /* TokenDot, */ {0, 0, 15, "."}, /* TokenArrow, */ {0, 0, 15, "->"},
...
void
ExpressionStackShow(Picoc * pc, struct ExpressionStack *StackTop)
{
...
int
IsTypeToken(struct ParseState *Parser, enum LexToken t, struct Value *LexValue)
{
	if (t >= TokenIntType && t <= TokenUnsignedType)
		return 1;	       /* base type */

	/* typedef'ed type? */
	if (t == TokenIdentifier)      /* see TypeParseFront, case
				        * TokenIdentifier and ParseTypedef */
	{
...
long
ExpressionCoerceInteger(struct Value *Val)
{
	switch (Val->Typ->Base)
	{
		case TypeInt:
...
unsigned long
ExpressionCoerceUnsignedInteger(struct Value *Val)
{
	switch (Val->Typ->Base)
	{
		case TypeInt:
...
#ifndef NO_FP
double
ExpressionCoerceFP(struct Value *Val)
{
...
long
ExpressionAssignInt(struct ParseState *Parser, struct Value *DestValue, long FromInt, int After)
{
	long	Result;
...
double
ExpressionAssignFP(struct ParseState *Parser, struct Value *DestValue, double FromFP)
{
	if (!DestValue->IsLValue)
		ProgramFail(Parser, "can't assign to this");
...
void
ExpressionStackPushValueNode(struct ParseState *Parser, struct ExpressionStack **StackTop, struct Value *ValueLoc)
{
...
struct Value *
ExpressionStackPushValueByType(struct ParseState *Parser, struct ExpressionStack **StackTop, struct ValueType *PushType)
{
...
void
ExpressionStackPushValue(struct ParseState *Parser, struct ExpressionStack **StackTop, struct Value *PushValue)
{
...
void
ExpressionStackPushLValue(struct ParseState *Parser, struct ExpressionStack **StackTop, struct Value *PushValue, int Offset)
{
...
void
ExpressionStackPushDereference(struct ParseState *Parser, struct ExpressionStack **StackTop, struct Value *DereferenceValue)
{
...
void
ExpressionPushInt(struct ParseState *Parser, struct ExpressionStack **StackTop, long IntValue)
{
...
#ifndef NO_FP
void
ExpressionPushFP(struct ParseState *Parser, struct ExpressionStack **StackTop, double FPValue)
{
...
/* assign to a pointer */
void
ExpressionAssignToPointer(struct ParseState *Parser, struct Value *ToValue, struct Value *FromValue, const char *FuncName, int ParamNo, int AllowPointerCoercion)
{
	...
	if (FromValue->Typ == ToValue->Typ || FromValue->Typ == Parser->pc->VoidPtrType || (ToValue->Typ == Parser->pc->VoidPtrType && FromValue->Typ->Base == TypePointer))
		ToValue->Val->Pointer = FromValue->Val->Pointer;	/* plain old pointer
									 * assignment */
...
void
ExpressionAssign(struct ParseState *Parser, struct Value *DestValue, struct Value *SourceValue, int Force, const char *FuncName, int ParamNo, int AllowPointerCoercion)
{
	...
	switch (DestValue->Typ->Base)
	{
		...
		case TypeArray:
			...
			{
				/* destination array is unsized - need to
				 * resize the destination array to the same
				 * size as the source array */
				DestValue->Typ = SourceValue->Typ;
				VariableRealloc(Parser, DestValue, TypeSizeValue(DestValue, FALSE));

				if (DestValue->LValueFrom != NULL)
				{
					/* copy the resized value back to the
					 * LValue */
			...
			{
				if (DestValue->Typ->ArraySize == 0)	/* char x[] = "abcd", x
									 * is unsized */
				{
...
void
ExpressionQuestionMarkOperator(struct ParseState *Parser, struct ExpressionStack **StackTop, struct Value *BottomValue, struct Value *TopValue)
{
...
void
ExpressionColonOperator(struct ParseState *Parser, struct ExpressionStack **StackTop, struct Value *BottomValue, struct Value *TopValue)
{
...
void
ExpressionPrefixOperator(struct ParseState *Parser, struct ExpressionStack **StackTop, enum LexToken Op, struct Value *TopValue)
{
	struct Value *Result;
...
void
ExpressionPostfixOperator(struct ParseState *Parser, struct ExpressionStack **StackTop, enum LexToken Op, struct Value *TopValue)
{
...
void
ExpressionInfixOperator(struct ParseState *Parser, struct ExpressionStack **StackTop, enum LexToken Op, struct Value *BottomValue, struct Value *TopValue)
{
	long	ResultInt = 0;
	struct Value *StackValue;
	void   *Pointer;
	...
	else if (Op == TokenAssign)
	{
		...
		HeapUnpopStack(Parser->pc, sizeof(struct Value));	/* XXX - possible bug if
									 * lvalue is a temp
									 * value and takes more
									 * than sizeof(struct
									 * Value) */
		ExpressionAssign(Parser, BottomValue, TopValue, FALSE, NULL, 0, FALSE);
		ExpressionStackPushValueNode(Parser, StackTop, BottomValue);
	...
	{
		/* cast a value to a different type *//* XXX - possible bug if
		 * the destination type takes more than sizeof(struct Value) +
		 * sizeof(struct ValueType *) */
...
/* take the contents of the expression stack and compute the top until there's
 * nothing greater than the given precedence */
void
ExpressionStackCollapse(struct ParseState *Parser, struct ExpressionStack **StackTop, int Precedence, int *IgnorePrecedence)
{
		...
		if (FoundPrecedence >= Precedence && TopOperatorNode != NULL)
		{
			...
			switch (TopOperatorNode->Order)
			{
				case OrderPrefix:
					...
					/* pop the value and then the prefix
					 * operator - assume they'll still be
				...
				case OrderPostfix:
					...
					/* pop the postfix operator and then
					 * the value - assume they'll still be
					 * there until we're done */
				...
				case OrderInfix:
					...
					if (TopValue != NULL)
					{
						...
						/* pop a value, the operator
						 * and another value - assume
...
void
ExpressionStackPushOperator(struct ParseState *Parser, struct ExpressionStack **StackTop, enum OperatorOrder Order, enum LexToken Token, int Precedence)
{
	struct ExpressionStack *StackNode = VariableAlloc(Parser->pc, Parser, sizeof(struct ExpressionStack), FALSE);
...
void
ExpressionGetStructElement(struct ParseState *Parser, struct ExpressionStack **StackTop, enum LexToken Token)
{
	struct Value *Ident;
...
int
ExpressionParse(struct ParseState *Parser, struct Value **Result)
{
	...
	do
	{
			...
			(Token != TokenColon || TernaryDepth > 0))
		{
			...
			if (PrefixState)
			{
				...
				{
					/* it's either a new bracket level or a
					 * cast */
					...
					if (IsTypeToken(Parser, BracketToken, LexValue) && (StackTop == NULL || StackTop->Op != TokenSizeof))
					{
						...
						/* scan and collapse the stack
						 * to the precedence of this
						 * infix cast operator, then
						 * push */
				...
				{
					/* scan and collapse the stack to the
					 * precedence of this operator, then
					 * push */
					/* take some extra care for double
					 * prefix operators, e.g. x = - -5, or
					 * x = **y */
					...
					if (NextToken > TokenComma && NextToken < TokenOpenBracket)
					{
						...
						/* two prefix operators with
						 * equal precedence? make sure
						 * the innermost one runs first */
						/* XXX - probably not correct,
						 * but can't find a test that
						 * fails at this */
			...
			else
			{
				...
				{
					/* scan and collapse the stack, then
					 * push */
					Precedence = BracketPrecedence + OperatorPrecedence[(int)Token].InfixPrecedence;
					/* for right to left order, only go
					 * down to the next higher precedence
					 * so we evaluate it in reverse order */
					/* for left to right order, collapse
					 * down to this precedence so we
					 * evaluate it in forward order */
					...
					{
						ExpressionGetStructElement(Parser, &StackTop, Token);	/* this operator is
													 * followed by a struct
													 * element so handle it
													 * as a special case */
					}
					else
					{
						/* if it's a && or || operator
						 * we may not need to evaluate
					...
					/* treat an open square bracket as an
					 * infix array index operator followed
					 * by an open bracket */
					if (Token == TokenLeftSquareBracket)
					{
						/* boost the bracket operator
						 * precedence, then push */
						BracketPrecedence += BRACKET_PRECEDENCE;
					}
				}
				else
		...
		else if (Token == TokenIdentifier)
		{
			...
			{
				if (Parser->Mode == RunModeRun	/* && Precedence <
					    IgnorePrecedence */ )
				{
					...
					{
						/* evaluate a macro as a kind
						 * of simple subroutine */
			...
			/* if we've successfully ignored the RHS turn ignoring
			 * off */
		...
		{
			/* it's a type. push it on the stack like a value. this
			 * is used in sizeof() */
...
void
ExpressionParseMacroCall(struct ParseState *Parser, struct ExpressionStack **StackTop, const char *MacroName, struct MacroDef *MDef)
{
...
#ifndef NO_FP
		ExpressionStackPushValueByType(Parser, StackTop, &Parser->pc->FPType);	/* largest return type
											 * there is */
#else
		ExpressionStackPushValueByType(Parser, StackTop, &Parser->pc->IntType);	/* largest return type
											 * there is */
#endif
...
/* do a function call */
void
ExpressionParseFunctionCall(struct ParseState *Parser, struct ExpressionStack **StackTop, const char *FuncName, int RunIt)
{
	struct Value *ReturnValue = NULL;
...
/* parse an expression */
long
ExpressionParseInt(struct ParseState *Parser)
{
	struct Value *Val;
	long	Result = 0;

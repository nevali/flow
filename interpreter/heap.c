...
void
HeapInit(Picoc * pc, int StackOrHeapSize)
{
	int	Count;
	int	AlignOffset = 0;
	...
	pc->HeapBottom = NULL;	       /* the bottom of the (downward-growing)
				        * heap */
	pc->StackFrame = NULL;	       /* the current stack frame */
	pc->HeapStackTop = NULL;       /* the top of the stack */
#else
#ifdef SURVEYOR_HOST
	pc->HeapMemory = (unsigned char *)C_HEAPSTART;	/* all memory - stack
							 * and heap */
	pc->HeapBottom = (void *)C_HEAPSTART + HEAP_SIZE;	/* the bottom of the
								 * (downward-growing)
								 * heap */
...
#else
	pc->HeapBottom = &HeapMemory[HEAP_SIZE];	/* the bottom of the
							 * (downward-growing)
							 * heap */
...
/* allocate some space on the stack, in the current stack frame clears memory.
 * can return NULL if out of stack space */
void   *
HeapAllocStack(Picoc * pc, int Size)
{
...
void
HeapUnpopStack(Picoc * pc, int Size)
{
#ifdef DEBUG_HEAP
...
int
HeapPopStack(Picoc * pc, void *Addr, int Size)
{
...
/* pop the current stack frame, freeing all memory in the frame. can return
 * NULL */
int
HeapPopStackFrame(Picoc * pc)
{
...
/* allocate some dynamically allocated memory. memory is cleared. can return
 * NULL if out of memory */
void   *
HeapAllocMem(Picoc * pc, int Size)
{
	...
	else if (pc->FreeListBig != NULL)
	{
		...
		if (*FreeNode != NULL)
		{
			...
			{
				/* close in size - reduce fragmentation by not
				 * splitting */
#ifdef DEBUG_HEAP
	...
	if (NewMem == NULL)
	{
		/* couldn't allocate from a freelist - try to increase the size
		 * of the heap area */
#ifdef DEBUG_HEAP

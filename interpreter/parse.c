...
enum ParseResult
ParseStatementMaybeRun(struct ParseState *Parser, int Condition, int CheckTrailingSemicolon)
{
...
int
ParseCountParams(struct ParseState *Parser)
{
	int	ParamCount = 0;
...
struct Value *
ParseFunctionDefinition(struct ParseState *Parser, struct ValueType *ReturnType, char *Identifier)
{
	...
	if (pc->TopStackFrame != NULL &&
		(!pc->TopStackFrame->ScopeType ||
			(pc->TopStackFrame->ScopeType->Base != TypeModule)))
	{
	...
	for (ParamCount = 0; ParamCount < FuncValue->Val->FuncDef.NumParams; ParamCount++)
	{
		...
		else
		{
			...
			{
				/* this isn't a real parameter at all - delete
				 * it */
				ParamCount--;
				FuncValue->Val->FuncDef.NumParams--;
			}
	...
	{
		/* it's a prototype, absorb the trailing semicolon */
	...
	else
	{
		...
		if (TableGet(Table, Identifier, &OldFuncValue, NULL, NULL, NULL))
		{
			if (OldFuncValue->Val->FuncDef.Body.Pos == NULL)
			{
				/* override an old function prototype */
				VariableFree(pc, TableDelete(pc, Table, Identifier));
			}
			else
	...
	if (!TableSet(pc, Table, Identifier, FuncValue, (char *)Parser->FileName, Parser->Line, Parser->CharacterPos))
		ProgramFail(Parser, "'%s' is already defined", Identifier);
...
int
ParseArrayInitialiser(struct ParseState *Parser, struct Value *NewVariable, int DoAssignment)
{
	int	ArrayIndex = 0;
	enum LexToken Token;
	struct Value *CValue;
	...
	while (Token != TokenRightBrace)
	{
		...
		else
		{
			...
			if (Parser->Mode == RunModeRun && DoAssignment)
			{
				...
				/* int x[3][3] = {1,2,3,4} => handle it just
				 * like int x[9] = {1,2,3,4} */
				while (ElementType->Base == TypeArray)
				{
					TotalSize *= ElementType->ArraySize;
					ElementType = ElementType->FromType;
					/* char x[10][10] = {"abc", "def"} =>
					 * assign "abc" to x[0], "def" to x[1]
					 * etc */
...
void
ParseDeclarationAssignment(struct ParseState *Parser, struct Value *NewVariable, int DoAssignment)
{
	struct Value *CValue;
...
int
ParseDeclaration(struct ParseState *Parser, enum LexToken Token)
{
	char   *Identifier;
	...
	TypeParseFront(Parser, &BasicType, &IsStatic, &Token);
	do
	{
		TypeParseIdentPart(Parser, BasicType, &Typ, &Identifier);
		if ((Token != TokenVoidType && Token != TokenStructType &&
				Token != TokenUnionType && Token != TokenEnumType &&
				Token != TokenInterface && Token != TokenClass &&
				Token != TokenModule) && Identifier == pc->StrEmpty)
...
void
ParseMacroDefinition(struct ParseState *Parser)
{
...
void
ParserCopy(struct ParseState *To, struct ParseState *From)
{
...
void
ParserCopyPos(struct ParseState *To, struct ParseState *From)
{
	To->Pos = From->Pos;
	To->Line = From->Line;
...
void
ParseFor(struct ParseState *Parser)
{
	int	Condition;
...
enum RunMode
ParseBlock(struct ParseState *Parser, int AbsorbOpenBrace, int Condition)
{
	int	PrevScopeID = 0, ScopeID = VariableScopeBegin(Parser, &PrevScopeID);
...
void
ParseTypedef(struct ParseState *Parser)
{
	struct ValueType *Typ;
...
/* parse a statement */
enum ParseResult
ParseStatement(struct ParseState *Parser, int CheckTrailingSemicolon)
{
	static int depth = 0;
	struct Value *CValue;
...
/*	fprintf(stderr, "%s:%d:%d: [%d] +++ ParseStatement begin +++\n", Parser->FileName, Parser->Line, Parser->CharacterPos, depth); */
...
/*	ParseDebugToken(Parser, Token); */
	...
	{
		case TokenEOF:
/*			fprintf(stderr, "%d: --- ParseStatement end (EOF) ---\n", depth); */
			depth--;
			return ParseResultEOF;

		case TokenIdentifier:
			/* might be a typedef-typed variable declaration or it
			...
			else
			{
				...
				{
					/* declare the identifier as a goto
					 * label */
					LexGetToken(Parser, NULL, TRUE);
					...
					/* new_identifier = something */
					/* /* try to guess type and declare the
				    variable based on assigned value */ if (NextToken == TokenAssign && !VariableDefinedAndOutOfScope(Parser->pc, LexerValue->Val->Identifier))
				{
					if (Parser->Mode == RunModeRun)
					{
						struct Value *CValue;
		...
		case TokenReturn:
			if (Parser->Mode == RunModeRun)
			{
				if (!Parser->pc->TopStackFrame || Parser->pc->TopStackFrame->ReturnValue->Typ->Base != TypeVoid)
				{
					...
					if (!Parser->pc->TopStackFrame)	/* return from top-level
									 * program? */
		...
		case TokenDelete:
			{
				/* try it as a function or variable name to
				 * delete */
				if (LexGetToken(Parser, &LexerValue, TRUE) != TokenIdentifier)
					ProgramFail(Parser, "identifier expected");
		...
		default:
			*Parser = PreState;
/*			fprintf(stderr, "%d: --- ParseStatement end (parse error) ---\n", depth); */
			depth--;
	...
	if (CheckTrailingSemicolon)
	{
		if (LexGetToken(Parser, NULL, TRUE) != TokenSemicolon)
		{
/*			fprintf(stderr, "%d: --- ParseStatement end (missing semicolon) ---\n", depth); */
			depth--;
			ProgramFail(Parser, "';' expected");
		}
	}
/*	fprintf(stderr, "%s:%d:%d: [%d] --- ParseStatement end ---\n", Parser->FileName, Parser->Line, Parser->CharacterPos, depth); */
	depth--;
...
void
PicocParse(Picoc * pc, const char *FileName, const char *Source, int SourceLen, int RunIt, int CleanupNow, int CleanupSource, int EnableDebugger)
{
...
/* parse interactively */
void
PicocParseInteractiveNoStartPrompt(Picoc * pc, int EnableDebugger)
{
	struct ParseState Parser;
	enum ParseResult Ok;
...
void
PicocParseInteractive(Picoc * pc)
{

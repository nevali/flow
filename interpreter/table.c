...
static unsigned int
TableHash(const char *Key, int Len)
{
	...
	{
		if ((unsigned long) Offset > sizeof(unsigned int) * 8 - 7)
...
/* initialise a table */
void
TableInitTable(struct Table *Tbl, struct TableEntry **HashTable, int Size, int OnHeap)
{
	Tbl->Size = Size;
	Tbl->OnHeap = OnHeap;
...
TableSearch(struct Table *Tbl, const char *Key, int *AddAt)
{
	struct TableEntry *Entry;
	int	HashValue = ((unsigned long)Key) % Tbl->Size;	/* shared strings have
								 * unique addresses so
...
/* set an identifier to a value. returns FALSE if it already exists. Key must
 * be a shared string from TableStrRegister() */
int
TableSet(Picoc * pc, struct Table *Tbl, char *Key, struct Value *Val, const char *DeclFileName, int DeclLine, int DeclColumn)
{
	int	AddAt;
...
/* find a value in a table. returns FALSE if not found. Key must be a shared
 * string from TableStrRegister() */
int
TableGet(struct Table *Tbl, const char *Key, struct Value **Val, const char **DeclFileName, int *DeclLine, int *DeclColumn)
{
	int	AddAt;
	struct TableEntry *FoundEntry = TableSearch(Tbl, Key, &AddAt);
...
struct Value *
TableDelete(Picoc * pc, struct Table *Tbl, const char *Key)
{
...
static struct TableEntry *
TableSearchIdentifier(struct Table *Tbl, const char *Key, int Len, int *AddAt)
{
	struct TableEntry *Entry;
...
char   *
TableSetIdentifier(Picoc * pc, struct Table *Tbl, const char *Ident, int IdentLen)
{
	int	AddAt;
	...
	{			       /* add it to the table - we economise by
				        * not allocating the whole structure
				        * here */
...
char   *
TableStrRegister2(Picoc * pc, const char *Str, int Len)
{
	return TableSetIdentifier(pc, &pc->StringTable, Str, Len);
}

char   *
TableStrRegister(Picoc * pc, const char *Str)
{

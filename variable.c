...
void
VariableFree(Picoc * pc, struct Value *Val)
{
...
void
VariableTableCleanup(Picoc * pc, struct Table *HashTable)
{
...
void
VariableCleanup(Picoc * pc)
{
	VariableTableCleanup(pc, &pc->GlobalTable);
	VariableTableCleanup(pc, &pc->StringLiteralTable);
}

/* allocate some memory, either on the heap or the stack and check if we've run
 * out */
void   *
VariableAlloc(Picoc * pc, struct ParseState *Parser, int Size, int OnHeap)
{
	void   *NewValue;
...
/* allocate a value either on the heap or the stack using space dependent on
 * what type we want */
struct Value *
VariableAllocValueAndData(Picoc * pc, struct ParseState *Parser, int DataSize, int IsLValue, struct Value *LValueFrom, int OnHeap)
{
...
struct Value *
VariableAllocValueFromType(Picoc * pc, struct ParseState *Parser, struct ValueType *Typ, int IsLValue, struct Value *LValueFrom, int OnHeap)
{
...
/* allocate a value either on the heap or the stack and copy its value. handles
 * overlapping data */
struct Value *
VariableAllocValueAndCopy(Picoc * pc, struct ParseState *Parser, struct Value *FromValue, int OnHeap)
{
...
/* allocate a value either on the heap or the stack from an existing AnyValue
 * and type */
struct Value *
VariableAllocValueFromExistingData(struct ParseState *Parser, struct ValueType *Typ, union AnyValue *FromValue, int IsLValue, struct Value *LValueFrom)
{
...
/* allocate a value either on the heap or the stack from an existing Value,
 * sharing the value */
struct Value *
VariableAllocValueShared(struct ParseState *Parser, struct Value *FromValue)
{
...
void
VariableRealloc(struct ParseState *Parser, struct Value *FromValue, int NewSize)
{
...
int
VariableScopeBegin(struct ParseState *Parser, int *OldScopeID)
{
	struct TableEntry *Entry;
	struct TableEntry *NextEntry;
	Picoc  *pc = Parser->pc;
...
void
VariableScopeEnd(struct ParseState *Parser, int ScopeID, int PrevScopeID)
{
	struct TableEntry *Entry;
	struct TableEntry *NextEntry;
	Picoc  *pc = Parser->pc;
	...
	for (Count = 0; Count < HashTable->Size; Count++)
	{
		for (Entry = HashTable->HashTable[Count]; Entry != NULL; Entry = NextEntry)
		{
			...
			if (Entry->p.v.Val->ScopeID == ScopeID && !Entry->p.v.Val->OutOfScope)
			{
				...
				Entry->p.v.Key = (char *)((intptr_t)Entry->p.v.Key | 1);	/* alter the key so it
												 * won't be found by
												 * normal searches */
			}
		}
...
int
VariableDefinedAndOutOfScope(Picoc * pc, const char *Ident)
{
	struct TableEntry *Entry;
	int	Count;
...
struct Value *
VariableDefine(Picoc * pc, struct ParseState *Parser, char *Ident, struct Value *InitValue, struct ValueType *Typ, int MakeWritable)
{
...
/* define a variable. Ident must be registered. If it's a redefinition from the
 * same declaration don't throw an error */
struct Value *
VariableDefineButIgnoreIdentical(struct ParseState *Parser, char *Ident, struct ValueType *Typ, int IsStatic, int *FirstVisit)
{
	Picoc  *pc = Parser->pc;
	...
	if (IsStatic)
	{
		...
		/* make the mangled static name (avoiding using sprintf() to
		 * minimise library impact) */
		...
		*MNPos++ = '/';
		...
		{
			/* define the mangled-named static variable store in
			 * the global scope */
...
int
VariableDefined(Picoc * pc, const char *Ident)
{
...
void
VariableGet(Picoc * pc, struct ParseState *Parser, const char *Ident, struct Value **LVal)
{
...
/* define a global variable shared with a platform global. Ident will be
 * registered */
void
VariableDefinePlatformVar(Picoc * pc, struct ParseState *Parser, char *Ident, struct ValueType *Typ, union AnyValue *FromValue, int IsWritable)
{
...
/* free and/or pop the top value off the stack. Var must be the top value on
 * the stack! */
void
VariableStackPop(struct ParseState *Parser, struct Value *Var)
{
	int	Success;
	...
	if (Var->ValOnHeap)
	{
		...
		Success = HeapPopStack(Parser->pc, Var, sizeof(struct Value));	/* value isn't our
										 * problem */
...
void
VariableStackFrameAdd(struct ParseState *Parser, const char *FuncName, int NumParams)
{
...
void
VariableStackFramePop(struct ParseState *Parser)
{
...
/* get a string literal. assumes that Ident is already registered. NULL if not
 * found */
struct Value *
VariableStringLiteralGet(Picoc * pc, char *Ident)
{
...
void
VariableStringLiteralDefine(Picoc * pc, char *Ident, struct Value *Val)
{
...
void   *
VariableDereferencePointer(struct ParseState *Parser, struct Value *PointerValue, struct Value **DerefVal, int *DerefOffset, struct ValueType **DerefType, int *DerefIsLValue)
{
	if (DerefVal != NULL)
		*DerefVal = NULL;
